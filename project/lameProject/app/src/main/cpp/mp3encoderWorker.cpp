...
int Mp3Encoder::Init(const char* pcmFilePath, const char* mp3FilePath, int sampleRate, int channels, int bitRate){
     ...
     void Mp3Encoder::Encode(){
        ...
        while ((readBufferSize = fread(buffer, 2, static_cast<size_t>(bufferSize / 2), pcmFile)) > 0){
            ...
            int  wroteSize = lame_encode_buffer(lameClient, leftBuffer, rightBuffer, readBufferSize/2, mp3_buffer, bufferSize);
            fwrite(mp3_buffer, 1, static_cast<size_t>(wroteSize), mp3File);
        }
